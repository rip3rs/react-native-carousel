...
class Carousel extends Component<Props, State> {
  ...
  pageType = (page: number, i: number): React$Element<typeof View> => {
    const { children } = this.props;
    const { index } = this.state;

    if (
      !React.isValidElement(children[page]) &&
      children[page].constructor === Object &&
      "attr" in children[page]
    ) {
      return (
        <View key={i}>
          {children[page].attr.isVideo === true && index !== page + 1 ? (
